...
Handlebars.registerHelper('divisionClass', function(division) {
  ...
  case 'Железо': return 'fe';
  case 'Хром': return 'cr';
  case 'Уголь': return 'c';
  case 'Энергетика': return 'e';
  default: return 'xx';
  }
  });

Handlebars.registerHelper('divisionSvg', function (division) {
  const normalized = String(division).replace(/[\u200B-\u200D\uFEFF\u00A0]/g, '').trim().toLowerCase().replace(/^["']+|["']+$/g, '');
  const map = {
    'алюминий': 'Al',
    'железо': 'Fe',
    'хром': 'Cr',
...
Handlebars.registerHelper('redflag', function (value) {
  ...
  if (typeof value === 'number') {
    ...
    num = parseFloat(value.replace(',', '.').replace(/\s/g, '').replace(/[^\d.-]/g, ''));
  } else {
    num = NaN;
  }
